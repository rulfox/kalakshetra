...
export function NavBar({
  ...
  return (
    ...
    >
      ...
      >
        <Wordmark tone={solid ? 'dark' : 'light'} size="md" as="a" href="#top" />

        <div className="ks-nav-desktop" style={{ display: 'flex', alignItems: 'center', gap: '2.4rem' }}>
          {links.map((l) => (
            <NavLink key={l.label} href={l.href} color={linkColor}>
              {l.label}
            </NavLink>
          ))}
          <WhatsAppButton
      ...
      >
        <div style={{ padding: 'var(--space-5) var(--gutter) var(--space-6)', display: 'flex', flexDirection: 'column', gap: '1.4rem' }}>
          ...
          <WhatsAppButton number={whatsappNumber} label="Order on WhatsApp" style={{ marginTop: '0.5rem' }} />
        </div>
      </div>
